...
Summoning also focuses the search box, via `MainWindow.FocusSearchBox()` — the same entry point Ctrl+F uses. Two things about it are not obvious. The call is **queued** on the dispatcher rather than made inline, because `BringToForeground` does its own work there and focus set before the window is actually foreground does not stick. And `Focus()` alone is not enough: whatever `FocusState` it is given, it lands the caret but leaves the selection collapsed after the old query, so summoning onto a stale "steam" and typing "chrome" yielded "steamchrome". The fix selects the text on the inner `TextBox`, reached with `FindDescendant<T>` because `AutoSuggestBox` has no selection API and the part lives in its template. Selecting rather than clearing keeps the previous results on screen for a summon that only wanted to look at them again. Only the hotkey does this; the tray icon deliberately does not, since a mouse click that then steals the caret is a nuisance.

...
**Changing what an existing key *means* is the case that rule does not cover, and the version stamp cannot carry it.** `WindowState` went from physical pixels to DIPs; a version-gated migration would have had to run before anything wrote the file, and `SaveConfig` stamps `CurrentVersion` on *every* write — including the one `MigrateOrInitialize` → `EnsureUsable` can fire at startup, and every immediate-apply settings change. Either would have relabelled a pixel value as a DIP one, permanently. Moving the conversion earlier is not available either: the data layer has no display to ask for a scale, and `LoadConfig`'s other callers have no business knowing about DPI. So the unit change rides on **new key names** (`WidthDip`/`HeightDip`) with the old pair kept as read-only migration input under `JsonIgnoreCondition.WhenWritingDefault` — key presence is self-describing, cannot be stamped wrong, and puts the conversion in the layer that has a scale.
...
**`AppWindow.Resize` and every `AppWindow.TitleBar` inset are in physical pixels**, while every XAML dimension is in DIPs. Divide by the scale. `XamlRoot.RasterizationScale` is not available in the constructor — measured: it is still null immediately after `InitializeComponent`, and the window has to be sized before it is shown or the user sees it resize itself — so initial sizing uses a `GetDpiForWindow` P/Invoke. Windows ships no DIP↔pixel converter to lean on: the Win32 DPI surface is queries and per-struct adjustments, its only conversion entry points (`LogicalToPhysicalPointForPerMonitorDPI` and its inverse) take a `POINT` rather than a size, and WinUI exposes the factor without anything that applies it. `WindowPlacement.ToPixels`/`ToDip` are the app's single pair, so the rounding is defined once.

**`OverlappedPresenter.PreferredMinimumWidth`/`Height` are physical pixels that Windows never rescales.** Set once from the launch display's scale, the floor is simply wrong everywhere else — measured on a 150%/100% pair: a window opened at 150% keeps a 1080×720px floor, which reads as 1080×720 DIP on the 100% display, half again the intended 720×480 and over half that screen's work area; the reverse direction leaves 480×320 DIP, under the width the title bar row is laid out for. The window *itself* is rescaled on the way across, so only the floor falls out of step. `ApplyMinimumSize` re-derives it; no resize is issued with it, because the floor is a constraint, not a size.

**A stale floor does not just misreport — it corrupts the crossing**, and this is what decides the mechanism. `WM_DPICHANGED` resizes the window by the DPI ratio, and that resize is clamped against whatever the floor currently says. Shrinking onto a lower-DPI display therefore lands wrong: measured with a synthetic drag, a 933×600 DIP window dragged from 150% to 100% came out **1080×720 DIP**, pinned to the floor it left behind. Growing is unaffected — a too-small floor cannot clamp — which is exactly the asymmetry the bug presents with.

**So the floor must be updated from `WM_DPICHANGED`, not `XamlRoot.Changed`.** The XamlRoot event reports the same change for far less machinery and was the first implementation, but it runs on the dispatcher, *after* the resize. A programmatic `SetWindowPos` across the boundary does not expose this — the dispatcher gets a turn first and it looks fixed — so it only fails on a real drag, where the message arrives inside a modal move loop. `DpiChangeWatcher` (`win/Services/`) subclasses the HWND and updates the floor ahead of `DefSubclassProc`, which is where the resize happens. Its subclass id differs from `HotkeyService`'s because both attach to the main HWND, and it is per-window with a `Detach` — the settings window is really destroyed, and HWNDs are recycled.

`SettingsWindow` crosses displays *during construction*: the OS creates it wherever it likes and `CenterOverOwner` moves it onto the owner's monitor. `WM_DPICHANGED` is sent synchronously from inside `AppWindow.Move`, so the watcher covers even that, but its floor is still seeded from the **owner's** scale up front — that is what makes the destination explicit, and it is already right when both displays share a scale and no message is sent. Its centring arithmetic uses the owner's scale too, since it has to centre the size the window will *land* at; only its `Resize` stays in its own scale, because the crossing multiplies that by the ratio and converting first would apply the ratio twice.

...
**Icon loading.** `IconService.GetIconAsync()` checks the disk cache, then extracts via `StorageFile.GetThumbnailAsync()`. The cache file is deleted when `FilePath` or `CustomIconPath` changes. A non-file path returns `null` and the templates fall back to a Segoe MDL2 glyph (`FallbackGlyph` / `IconVisibility` / `FallbackIconVisibility`).
...
**Localization.** `Loc.GetString(key)` tries `ResourceLoader` first (MSIX), then falls back to embedded `.resw` files parsed via `XDocument` — which is the path this unpackaged build actually uses, and also why `x:Uid` does not work here. Language comes from `Loc.Initialize` (the `AppSettings` override) or `CultureInfo.CurrentUICulture`: `zh`→Chinese, `ja`→Japanese, else English. All three of `win/Strings/{en-US,zh-TW,ja-JP}/Resources.resw` must be updated together. Adding a *string* needs no `.csproj` change; adding a new *language file* does, as an `EmbeddedResource` with a matching `LogicalName`.

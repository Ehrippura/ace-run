...
合併階段之後**要重新編號把缺口補起來**，但必須同時更新所有交叉引用：`doc/spec.md` 的表格、各階段檔案內文提到的「第 N 階段」、以及程式碼註解中的檔名（例如 `Models/AppSettings.cs` 指向 `10-settings.md`）。`doc/CHANGELOG_*.md` 是歷史記錄，措辭不改，但指向已改名檔案的連結要修。

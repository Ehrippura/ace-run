...
這條規則是有代價才學到的：第九階段「多標籤支援」整篇都在解除第六階段的單標籤限制，而第六階段檔案卻始終寫著「V1 UI 為單一標籤」，只讀該檔的人會被誤導。該階段已併回第六階段，因此**編號有缺口是正常的** — 不要為了補齊而重新編號，那會讓所有既有連結與 commit 訊息失效。

歷史（某件事何時做的）由 `doc/CHANGELOG_*.md` 與 git 承擔，不需要由階段檔案保存。
...
| 第六階段：Tag 標籤管理（含多標籤支援，原第九階段） | [6-tags.md](spec/6-tags.md) | ✓ 已完成 |

...
- [x] `IconService.ClearCache()` 清空整個快取目錄，單一檔案刪不掉就跳過而非中止，並回傳實際清除的數量。同時清空進行中的擷取表。
  - 不做過濾：快取檔沒有副檔名（見第三階段 §1），沒有可比對的樣式可用，而這個目錄從頭到尾只有 `IconService` 會寫入。全掃也順帶收掉兩種過濾會跳過的殘骸——擷取中途失敗留下的 `.tmp`，以及改名前的 `<guid>.png`（任何查詢都已經構不到它們）。這也使這顆按鈕成為該次改名唯一的 migration 路徑。
